                          ...
                          void* user_data)
{
    ...
    {
        ERR("buffer underrun (buffer size=%d) (only buffering %d)\n", buffer_size_in_bytes, available_bytes);
        /* Zero the part of the buffer that we cannot fill */
        memset((char*)sample_buffer + available_bytes, 0, buffer_size_in_bytes - available_bytes);
        buffer_size_in_bytes = available_bytes;
    }
                                      ...
                                      const ALCchar *deviceName)
{
    ...
    {
        ERR("PPAPI only supports 44100 and 44800 sample frequencies\n");
        return ALC_FALSE;
    }
